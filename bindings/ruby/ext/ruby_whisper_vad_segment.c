...
static void rb_whisper_vad_segment_mark(void* p) {
    ruby_whisper_vad_segment* rwvs = (ruby_whisper_vad_segment*)p;
    rb_gc_mark(rwvs->segments);
}

static size_t ruby_whisper_vad_segment_memsize(const void* p) {
...
static const rb_data_type_t ruby_whisper_vad_segment_type = {"ruby_whisper_vad_segment",
                                                             {
                                                                 rb_whisper_vad_segment_mark,
                                                                 RUBY_DEFAULT_FREE,
                                                                 ruby_whisper_vad_segment_memsize,
                                                             },
                                                             0,
                                                             0,
                                                             0};

static VALUE ruby_whisper_vad_segment_s_allocate(VALUE klass) {
...
static VALUE ruby_whisper_vad_segment_get_start_time(VALUE self) {
    ruby_whisper_vad_segment* rwvs;
...
static VALUE ruby_whisper_vad_segment_get_end_time(VALUE self) {
    ruby_whisper_vad_segment* rwvs;
    ruby_whisper_vad_segments* rwvss;
    float t1;
...
static VALUE ruby_whisper_vad_segment_deconstruct_keys(VALUE self, VALUE keys) {
    ruby_whisper_vad_segment* rwvs;
    ruby_whisper_vad_segments* rwvss;
    VALUE hash, key;
    long n_keys;
    ...
    hash = rb_hash_new();
    if (NIL_P(keys)) {
        keys = rb_ary_new3(N_KEY_NAMES, sym_start_time, sym_end_time);
        n_keys = N_KEY_NAMES;
    } else {
...
void init_ruby_whisper_vad_segment(VALUE* mVAD) {
