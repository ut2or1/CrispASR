...
```bash
# Russian ASR:
crispasr --backend fastconformer-ctc -m stt-ru-fastconformer-hybrid-ctc-large-q4_k.gguf -f audio.wav

# Forced alignment (word timestamps for known text, or re-timing an .srt):
crispasr --align-only -am stt-ru-fastconformer-hybrid-ctc-large-q4_k.gguf \
