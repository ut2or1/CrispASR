...
llm_build_granite::llm_build_granite(const llama_model& model, const llm_graph_params& params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        cur = build_attention_layer(cur, inp_pos, inp_attn, model, n_embd_head, il);
...
ggml_tensor* llm_build_granite::build_attention_layer(ggml_tensor* cur, ggml_tensor* inp_pos,
                                                      llm_graph_input_attn_kv* inp_attn, const llama_model& model,
                                                      const int64_t n_embd_head, const int il) {
...
ggml_tensor* llm_build_granite::build_layer_ffn(ggml_tensor* cur, ggml_tensor* inpSA, const llama_model& model,
                                                const int il) {
