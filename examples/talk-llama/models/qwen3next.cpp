...
ggml_tensor* llm_build_qwen3next::build_norm_gated(ggml_tensor* input, ggml_tensor* weights, ggml_tensor* gate,
                                                   int layer) {
...
ggml_tensor* llm_build_qwen3next::build_layer_attn(llm_graph_input_attn_kv* inp, ggml_tensor* cur, ggml_tensor* inp_pos,
                                                   int il) {
    ...
    ggml_tensor* Qcur = ggml_view_4d(ctx0, Qcur_full, n_embd_head, n_head, n_tokens, 1, Qcur_full->nb[1],
                                     Qcur_full->nb[2], Qcur_full->nb[3], 0);
    cb(Qcur, "Qcur_view", il);

    ggml_tensor* gate = ggml_view_4d(ctx0, Qcur_full, n_embd_head, n_head, n_tokens, 1, Qcur_full->nb[1],
                                     Qcur_full->nb[2], Qcur_full->nb[3], n_embd_head * ggml_element_size(Qcur_full));
    cb(gate, "gate", il);
...
std::pair<ggml_tensor*, ggml_tensor*> llm_build_qwen3next::build_qkvz(ggml_tensor* input, int il) {
    ...
    if (model.layers[il].wqkv) {
        ...
        ggml_tensor* mixed_qkvz_reshaped =
            ggml_reshape_4d(ctx0, mixed_qkvz, qkvz_new_dim, num_k_heads, n_seq_tokens, n_seqs);
        ...
        ggml_tensor* key =
            ggml_view_4d(ctx0, mixed_qkvz_reshaped, split_sizes_qkvz[1], num_k_heads, n_seq_tokens, n_seqs,
        ...
        ggml_tensor* z = ggml_view_4d(
            ctx0, mixed_qkvz_reshaped, split_sizes_qkvz[3], num_k_heads, n_seq_tokens, n_seqs,
            mixed_qkvz_reshaped->nb[1], mixed_qkvz_reshaped->nb[2], mixed_qkvz_reshaped->nb[3],
...
ggml_tensor* llm_build_qwen3next::build_layer_attn_linear(llm_graph_input_rs* inp, ggml_tensor* cur, int il) {
    ...
    if (model.layers[il].ffn_gate_inp != nullptr) {
        ...
        ggml_tensor* moe_out =
            build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                          ...
                          n_expert_used, LLM_FFN_SILU, true, hparams.expert_weights_scale,
                          LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il, nullptr, model.layers[il].ffn_gate_up_exps);
