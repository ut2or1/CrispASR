...
ggml_tensor* llm_build_nemotron_h::build_attention_layer(ggml_tensor* cur, llm_graph_input_attn_kv* inp_attn,
...
ggml_tensor* llm_build_nemotron_h::build_ffn_layer(ggml_tensor* cur, const llama_model& model, int il) {
    if (model.layers[il].ffn_gate_inp == nullptr) {
        ...
        ggml_tensor* moe_out =
            build_moe_ffn(inp_latent, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                          nullptr, // no gate
                          model.layers[il].ffn_down_exps, model.layers[il].ffn_exp_probs_b, n_expert, n_expert_used,
                          LLM_FFN_RELU_SQR, hparams.expert_weights_norm, hparams.expert_weights_scale,
                          LLAMA_EXPERT_GATING_FUNC_TYPE_SIGMOID, il, router_logits);
        ...
        ggml_tensor* ffn_shexp =
            build_ffn(inp_emb, model.layers[il].ffn_up_shexp, NULL, NULL, NULL /* no gate */, NULL, NULL,
                      model.layers[il].ffn_down_shexp, NULL, NULL, NULL, LLM_FFN_RELU_SQR, LLM_FFN_PAR, il);
