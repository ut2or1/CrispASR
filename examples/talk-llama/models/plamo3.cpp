...
llm_build_plamo3<iswa>::llm_build_plamo3(const llama_model& model, const llm_graph_params& params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        ggml_tensor* Qcur = ggml_view_3d(ctx0, qkv, head_dim_q, n_head, n_tokens, head_dim_q * sizeof(float),
                                         qkv->nb[1], q_offset * ggml_element_size(qkv));
        ggml_tensor* Kcur = ggml_view_3d(ctx0, qkv, head_dim_q, n_head_kv, n_tokens, head_dim_q * sizeof(float),
                                         qkv->nb[1], k_offset * ggml_element_size(qkv));
        ggml_tensor* Vcur = ggml_view_3d(ctx0, qkv, head_dim_v, n_head_kv, n_tokens, head_dim_v * sizeof(float),
        ...
        cur = build_attn(inp_attn, model.layers[il].wo, NULL, Qcur, Kcur, Vcur, nullptr, nullptr, nullptr, attn_scale,
                         il);
        cb(cur, "attn_out", il);
