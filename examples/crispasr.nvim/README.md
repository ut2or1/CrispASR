...
- Place the [crispasr.nvim](crispasr.nvim) script somewhere in your PATH and give it execute permissions:

  ```
  cp examples/crispasr.nvim/crispasr.nvim ~/bin/
  chmod u+x ~/bin/crispasr.nvim
  ```

- Fine-tune the script to your preference and machine parameters:

  ```
  ./stream -t 8 -m models/ggml-base.en.bin --step 350 --length 10000 -f /tmp/crispasr.nvim 2> /dev/null
  ```
...
- Add the following shortcuts to your `~/.config/nvim/init.vim`:
  ...
  ```
  inoremap <C-G>  <C-O>:!crispasr.nvim<CR><C-O>:let @a = system("cat /tmp/crispasr.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR><C-R>a
  nnoremap <C-G>       :!crispasr.nvim<CR>:let @a = system("cat /tmp/crispasr.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR>"ap
  vnoremap <C-G> c<C-O>:!crispasr.nvim<CR><C-O>:let @a = system("cat /tmp/crispasr.nvim \| tail -n 1 \| xargs -0 \| tr -d '\\n' \| sed -e 's/^[[:space:]]*//'")<CR><C-R>a
  ```
  
  Explanation: pressing `Ctrl-G` runs the [crispasr.nvim](crispasr.nvim) script which in turn calls the `stream` binary to transcribe your speech through the microphone. The results from the transcription are continuously dumped into `/tmp/crispasr.nvim`. After you kill the program with `Ctrl-C`, the vim command grabs the last line from the `/tmp/crispasr.nvim` file and puts it under the cursor.
