...
constexpr char operator""_P(const char* c, size_t size) {
    return size < 2 || RANK < 0 || RANK > 7 || FILE < 0 || FILE > 7 ? INVALID_POS : FILE * 8 + RANK;
}
#undef FILE
#undef RANK
...
char strToType(sview str) {
    auto it = std::find_if(pieceNames.begin(), pieceNames.end(),
                           [str](const char* name) { return strncmp(name, str.ptr, str.size) == 0; });
...
template <class Modifier> char traverse(char pos, const Direction& d, const Modifier& m, int count = 8) {
    while (--count >= 0) {
        pos = makeStep(pos, d);
        if (pos == INVALID_POS || m(pos))
            break;
    }
    return pos;
}
...
std::vector<Direction> filter(const Direction& pin, std::initializer_list<Direction> directions) {
    ...
    for (auto& d : directions) {
        if ((d[R] == pin[R] || d[R] == -pin[R]) && (d[F] == pin[F] || d[F] == -pin[F]))
            result.push_back(d);
    }
    return result;
}
...
struct Pawn : public Piece {
    ...
    bool is_first_move() const { return m_color ? coord()[F] == '7' : coord()[F] == '2'; }
...
struct Queen : public Piece {
    ...
    virtual bool movePattern(char pos) const override {
        ...
        auto cur = coord();
        auto next = positions[pos];
        return cur[R] == next[R] || cur[F] == next[F] || cur[R] - cur[F] == next[R] - next[F] ||
               cur[R] + cur[F] == next[R] + next[F];
    }
...
struct Add {
    Add(const Board& board, std::set<char>& moves, Piece::Colors color)
        : m_board(board), m_moves(moves), m_color(color) {}
    bool operator()(char pos) const {
        if (!m_board[pos] || m_board[pos]->color() != m_color)
            m_moves.insert(pos);
        return m_board[pos];
    }
    const Board& m_board;
...
void Pawn::reinit(const State& state) {
    ...
    for (auto& direction : filter(pin, {left, right})) {
        ...
        if (pos != INVALID_POS && state.board[pos] && state.board[pos]->color() != m_color)
            m_allowed.insert(pos);
    }
...
Chessboard::Chessboard() : m_state(new State()) {
    setGrammar();
}
...
void Chessboard::setGrammar() {
    ...
    for (auto& p : pieceTypes)
        result += " \"" + std::string(pieceNames[p]) + "\" |";
    result.pop_back();
    result += ")\n\n";

    result += "frompos ::= (";
    for (auto& p : from_pos)
        result += " \"" + std::string(positions[p]) + "\" |";
    result.pop_back();
    result += ")\n";
...
std::string Chessboard::process(const std::string& command) {
    ...
    if (!parseCommand(command, piece, pos_to))
        return "";

    auto pos_from = piece->pos();

    if (!move(*piece, pos_to))
        return "";
    ...
    for (auto& p : enemyPieces)
        p.reinit(*m_state); // only enemy moves needed next

    std::string result = {positions[pos_from][R], positions[pos_from][F], '-', positions[pos_to][R],
                          positions[pos_to][F]};
    ++m_moveCounter;
    setGrammar();
...
bool Chessboard::parseCommand(const std::string& command, Piece*& piece, char& pos_to) {
    auto color = Piece::Colors(m_moveCounter % 2);
    fprintf(stdout, "%s: Command to %s: '%s%.*s%s'\n", __func__, (color ? "Black" : "White"), "\033[1m",
            int(command.size()), command.data(), "\033[0m");
    ...
    if (tokens.size() == 1) {
        ...
        if (pos_from == INVALID_POS)
            type = Piece::Types(strToType(tokens.front()));
        pos_to = strToPos(tokens.back());
    }
    if (pos_to == INVALID_POS)
        return false;
    if (pos_from == INVALID_POS) {
        if (type == Piece::Types::NUM_PIECES)
            return false;
    ...
    if (pos_from == INVALID_POS)
        return false;
    if (m_state->board[pos_from] == nullptr)
        return false;
...
void Chessboard::updatePins(Piece& piece) {
    if (piece.type() == Piece::Pawn || piece.type() == Piece::Knight || piece.type() == Piece::King)
        return;
...
bool Chessboard::move(Piece& piece, char pos_to) {
    ...
    if (allowed.count(pos_to) == 0 || (m_inCheck && piece.type() != Piece::King && m_allowedInCheck.count(pos_to) == 0))
        return false;
    if (m_state->board[pos_to] && m_state->board[pos_to]->color() == piece.color())
        return false;
    if (m_state->board[pos_to])
        m_state->board[pos_to]->take();
