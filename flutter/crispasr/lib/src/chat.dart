...
class CrispasrChatSession {
  CrispasrChatSession._(
      this._lib, this._handle, this._templateName, this._nCtx) {
  ...
  static final Finalizer<Pointer<Void>> _finalizer =
    ...
    try {
      ...
      final close = lib.lookupFunction<Void Function(Pointer<Void>),
  ...
  Future<String> generate(
    ...
    try {
      final out = generate(
          _handle, marshalled.messagesPtr, messages.length, paramsPtr, errPtr);
      if (out == nullptr) {
        throw ChatException(errPtr.ref.code, _readErrorMessage(errPtr));
      }
      try {
